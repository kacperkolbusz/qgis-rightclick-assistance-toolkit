"""
Add Hyperlink to Polygon Action for Right-click Utilities and Shortcuts Hub

Adds a clickable hyperlink to polygon features. User can input a URL that will be stored
...
class HyperlinkInputDialog(QDialog):
    ...
    def __init__(self, parent=None, current_url=None, field_name="hyperlink", can_open=True):
        ...
        self.setWindowTitle("Add/Open Hyperlink to Polygon")
        self.setModal(True)
        self.resize(500, 200)
...
class AddHyperlinkPolygonAction(BaseAction):
    """
    Action to add a clickable hyperlink to polygon features.
    
    This action allows users to associate a URL with a polygon feature. The URL is stored
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "add_hyperlink_polygon"
        self.name = "Add/Open Hyperlink to Polygon"
        self.category = "Editing"
        self.description = "Add or open a clickable hyperlink for the selected polygon feature. User can input a URL that will be stored with the feature. Option to open the link immediately in browser. After setting, you can activate the click tool to click on features and open their hyperlinks."
        self.enabled = True
    ...
    def execute(self, context):
        ...
        if confirm_before_adding:
            if url:
                message = f"Set hyperlink for polygon feature ID {feature.id()}?\n\nURL: {url}"
            else:
                message = f"Remove hyperlink from polygon feature ID {feature.id()}?"
        ...
        try:
            ...
            if show_success:
                if url:
                    self.show_info("Success", f"Hyperlink added to polygon feature ID {feature.id()}\n\nURL: {url}")
                else:
                    self.show_info("Success", f"Hyperlink removed from polygon feature ID {feature.id()}")
...
add_hyperlink_polygon = AddHyperlinkPolygonAction()
