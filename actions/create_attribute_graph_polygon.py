"""
Create Attribute Graph for Polygon Layer Action for Right-click Utilities and Shortcuts Hub

Creates a bar graph visualization of numeric attributes for polygon features.
    ...
    def update_graph(self):
        ...
        with plt.style.context(self.style_preset):
            ...
            if display_values:
                ...
                if self.horizontal:
                    # For horizontal bars, y-axis shows feature IDs
                    if len(display_ids) > 20:
                        step = max(1, len(display_ids) // 20)
                        ax.set_yticks(range(0, len(display_ids), step))
                        ax.set_yticklabels([display_ids[i] for i in range(0, len(display_ids), step)])
                    else:
                        ax.set_yticks(range(len(display_ids)))
                        ax.set_yticklabels(display_ids)
                else:
                    # For vertical bars, x-axis shows feature IDs
                    if len(display_ids) > 20:
                        step = max(1, len(display_ids) // 20)
...
class CreateAttributeGraphPolygonAction(BaseAction):
    """Action to create attribute graphs for polygon features."""
    ...
    def __init__(self):
        ...
        # Required properties
        self.action_id = "create_attribute_graph_polygon"
        self.name = "Create Attribute Graph"
        self.category = "Analysis"
        self.description = "Create a bar graph visualization of numeric attributes for polygon features. Allows selection of which numeric field to visualize, displaying values for each feature in the layer. Supports exporting graphs, visual presets, and sorting/filtering options."
        self.enabled = True
...
create_attribute_graph_polygon_action = CreateAttributeGraphPolygonAction()